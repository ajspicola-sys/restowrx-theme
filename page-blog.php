<?php
/**
 * Hot Water Heroes Plumbing — Blog Page Template
 * Auto-loaded by WordPress for any page with the slug 'blog'.
 * Uses WP_Query — no Settings > Reading configuration required.
 */
get_header();

// Build the posts query — handles pagination and category filtering
$current_cat = get_query_var('cat') ?: (isset($_GET['cat']) ? intval($_GET['cat']) : 0);
    ...
    <section class="blog-filters" aria-label="Filter by category">
        <div class="section__inner">
            <div class="blog-filters__bar">
                ...
                ?>
                    <a href="<?php echo esc_url(add_query_arg('cat', $cat->term_id, $blog_url)); ?>"
    ...
    <section class="blog-archive" aria-label="Blog posts">
        <div class="section__inner">
            <?php if ($blog_query->have_posts()) : ?>
                ...
                <!-- Featured Post (First post, non-filtered, page 1 only) -->
                <?php if (!$current_cat && $paged <= 1 && $blog_query->have_posts()) : $blog_query->the_post(); $showed_featured = true; ?>
                ...
                <div class="blog-grid">
                    <?php
                    // Rewind so grid starts from post 1
                    $blog_query->rewind_posts();
                    $grid_count = 0;
                    while ($blog_query->have_posts()) : $blog_query->the_post();
                        $grid_count++;
                        // Skip post 1 if already shown as featured
                ...
                <nav class="blog-pagination" aria-label="Blog pagination">
                    <?php
                    echo paginate_links([
                        'base'      => trailingslashit($blog_url) . '%_%',
                        'format'    => 'page/%#%/',
                        'current'   => $paged,
